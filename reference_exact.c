...
typedef enum status_t {
    SUCCESS, SMALL_GRAPH_ERR, LARGE_GRAPH_ERR, EOF_GRAPH_ERR,
...
unsigned int bit[] = {020000000000, 010000000000, 04000000000, 02000000000, 01000000000,
...
inline void set_u_blue(int G[][MMAX], int n, int u, int num_dominated[], int n_dominated, int dom[],
    ...
    if (!undominatable)
        find_dom_set(G, n, num_dominated, n_dominated, dom, min_size, min_dom, num_choice, level + 1, size, delta,
                     num_tests);
...
inline void set_u_red(int G[][MMAX], int n, int u, int num_dominated[], int n_dominated, int dom[],
    ...
    dom[size] = u;
    find_dom_set(G, n, num_dominated, n_dominated, dom, min_size, min_dom, num_choice, level + 1, size + 1, delta,
                 num_tests);
...
void find_dom_set(int G[][MMAX], int n, int num_dominated[], int n_dominated, int dom[],
    ...
    if (deg_u == 1) {
        set_u_red(G, n, u, num_dominated, n_dominated, dom, min_size, min_dom, num_choice, level, size, delta,
                  num_tests);
        return;
    }
        ...
        */
        ...
        set_u_blue(G, n, u, num_dominated, n_dominated, dom, min_size, min_dom, num_choice, level, size, delta,
                   num_tests);
        set_u_red(G, n, u, num_dominated, n_dominated, dom, min_size, min_dom, num_choice, level, size, delta,
                  num_tests);
        return;
...
*/
void
read_graph(int *n, int *m, int G[NMAX][MMAX], status_t *stat, int *bad, int *min_degree, int *max_degree, int *maxv) {
    int i, j;
    int deg, v;
